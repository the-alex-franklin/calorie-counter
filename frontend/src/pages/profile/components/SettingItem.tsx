	...
	return (
		<div className={`flex items-center justify-between p-3 mb-3 rounded-xl cursor-pointer bg-card border dark:border-gray-800 border-gray-100`} onClick={onClick || onToggle}>
          ...
          ${danger ? "bg-red-500" : "bg-primary bg-opacity-20"}`}
			...
			{toggle
				? (
					<div className={`w-12 h-6 rounded-full relative transition-colors ${isToggled ? "bg-primary" : "dark:bg-gray-700 bg-gray-300"}`}>
						<div className={`absolute w-5 h-5 bg-white rounded-full top-0.5 transition-all ${isToggled ? "right-0.5" : "left-0.5"}`} />
					</div>
				)
				: value
				? <span className={`text-sm dark:text-gray-400 text-gray-500`}>{value}</span>
